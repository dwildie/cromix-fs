...
public class Inode {
    ...
    public static Inode from(byte[] raw) {
        ...
        if (inode.type == CHARACTER_DEVICE || inode.type == BLOCK_DEVICE) {
            ...
            inode.directoryEntryCount = 0xFFFF & readWord(raw, INODE_DIR_COUNT_OFFSET);
        }
        inode.fileSize = readDWord(raw, INODE_F_SIZE_OFFSET);
        inode.usedBlockCount = readDWord(raw, INODE_USED_BLOCKS_OFFSET);
        inode.ownerPermission = 0xFF & raw[INODE_P_OWN_OFFSET];
    ...
    public void toBytes(byte[] data, int offset) {
        ...
        data[offset + INODE_P_OWN_OFFSET] = (byte)ownerPermission;
        ...
        if (type == CHARACTER_DEVICE || type == BLOCK_DEVICE) {
            ...
            writeWord(directoryEntryCount, data, offset + INODE_DIR_COUNT_OFFSET);
        }
        ...
        data[offset + INODE_LINKS_OFFSET + 1] = (byte)0;
